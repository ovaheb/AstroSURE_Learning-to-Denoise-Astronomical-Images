...
def sample_cr_params(
    ...
    if min_dEdx is None:
        min_dEdx = cr["min_dEdx"]
    if max_dEdx is None:
        max_dEdx = cr["max_dEdx"]
    if min_cr_len is None:
        min_cr_len = cr["min_cr_len"]
    if max_cr_len is None:
        max_cr_len = cr["max_cr_len"]
    if grid_size is None:
        grid_size = cr["grid_size"]
...
def simulate_crs(
    ...
    if conversion_factor is None:
        conversion_factor = cr["conversion_factor"]
    if pixel_size is None:
        pixel_size = cr["pixel_size"]
    if pixel_depth is None:
        pixel_depth = cr["pixel_depth"]
